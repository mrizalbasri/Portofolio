          ...
          {post.images && post.images.length > 0 ? (
            <div className="mb-12 space-y-6">
              ...
              {post.images.length > 1 && (
                <div
                  className={`grid gap-6 ${post.images.length === 2 ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1 md:grid-cols-2"}`}
                >
                  {post.images.slice(1).map((img, idx) => (
                    <OptimizedImage
                      ...
                      containerClassName="rounded-xl overflow-hidden group hover:border-cyan-500/50 transition-all cursor-pointer"
                    />
                  ))}
                </div>
              )}
              ...
              >
                <ReactMarkdown
                  components={{
                    h2: ({ ...props }) => (
                      <h2
                        className="text-4xl font-bold text-white mb-8 mt-16 pb-4 border-b border-white/10"
                        {...props}
                      />
                    ),
                    h3: ({ ...props }) => (
                    ...
                    ul: ({ ...props }) => (
                      <ul
                        className="text-gray-300 space-y-2 my-6 list-disc pl-6"
                        {...props}
                      />
                    ),
                    li: ({ ...props }) => (
              ...
              <div className="border-t border-white/10 pt-8">
                ...
                <div className="flex flex-wrap gap-3">
                  {post.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-gray-300 hover:border-cyan-500/50 hover:text-cyan-400 transition-all cursor-pointer"
                    >
                      #{tag}
                    </span>
                  ))}
